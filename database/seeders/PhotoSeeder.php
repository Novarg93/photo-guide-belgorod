...
class PhotoSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        Example::query()->select(['id', 'category_id'])->get()->each(function (Example $example) use ($placeholderPath): void {
            Photo::query()->updateOrCreate(
                [
                    'example_id' => $example->id,
                    'source_type' => 'stock',
                    'source_url' => "https://example.com/library/{$example->id}",
                ],
                [
                    'title' => "Placeholder photo for Example {$example->id}",
                    'category_id' => $example->category_id,
                    'path' => $placeholderPath,
                    'author_name' => 'Placeholder Library',
                    'license' => 'stock',
                    'permission_note' => 'Seeded placeholder image for local development.',
                    'is_active' => true,
                ],
            );
